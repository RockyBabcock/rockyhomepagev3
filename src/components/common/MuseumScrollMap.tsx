...
const sections = [
  ...
  { id: "projects", label: "Project Laboratory", num: "01" },
  { id: "forge", label: "Capability Forge", num: "02" },
  { id: "experiments", label: "Experiment Chambers", num: "03" },
  { id: "archives", label: "Personal Archives", num: "04" },
  { id: "garden", label: "Digital Garden", num: "05" },
  { id: "signal", label: "Signal Room", num: "06" }
];
    ...
    <div className="museum-scroll-map fixed top-1/2 -translate-y-1/2 left-4 z-50 flex-col gap-6 hidden xl:flex pointer-events-none">
      <div className="bg-[var(--museum-paper)]/80 backdrop-blur-md p-4 rounded-2xl border border-[var(--border)] shadow-sm pointer-events-auto">
        ...
        <motion.div 
          ...
          initial={false}
          animate={{
            height: `${Math.max(0, sections.findIndex(s => s.id === activeSection)) * (100 / (sections.length - 1))}%`
          }}
            ...
            return (
              <div key={section.id} className="relative flex items-center group">
                <a 
                  ...
                  onClick={(e) => {
                    e.preventDefault();
                    const el = section.id === 'entrance' ? document.getElementById('entrance-wrapper') : document.getElementById(section.id);
                    if(el) {
                ...
                >
                  <div className={cn(
                    ...
                    isActive ? "border-[var(--orange)] shadow-[0_0_12px_rgba(255,159,28,0.4)]" : "border-[var(--line-strong)] hover:border-[var(--ink)]"
                  )}>
                    <div className={cn(
                      "w-1.5 h-1.5 rounded-full transition-all duration-300",
                      isActive ? "bg-[var(--orange)] scale-100" : "bg-transparent scale-0"
                    )} />
                  </div>
                  ...
                  <div className={cn(
                    ...
                    <span className="text-[9px] font-mono font-bold text-[var(--ink-muted)] mb-0.5">HALL {section.num}</span>
                    <span className={cn(
                      "text-[10px] font-mono tracking-widest uppercase transition-colors whitespace-nowrap",
                      isActive ? "text-[var(--ink)] font-bold" : "text-[var(--ink-soft)] font-medium"
                    )}>
                      {section.label}
                    </span>
                  </div>
