...
import { ExternalLink, Github, Terminal, Activity, Box, Database, Sparkles, Layers } from "lucide-react";
    ...
    return (
      <div className="w-full h-48 bg-[#FAFAFA] border border-[var(--border)] rounded-xl flex items-center justify-center p-4 gap-4 overflow-hidden relative">
        ...
        <div className="flex flex-col gap-2 z-10 w-1/3">
          <div className="bg-white border rounded-lg p-2 text-[10px] font-mono text-center shadow-sm">Routing Layer</div>
          <div className="bg-white border rounded-lg p-2 text-[10px] font-mono text-center shadow-sm border-l-2 border-l-[var(--hall-primary)]">Hero Entrance</div>
          <div className="bg-white border rounded-lg p-2 text-[10px] font-mono text-center shadow-sm border-l-2 border-l-[var(--orange)]">Capability Forge</div>
        </div>
        ...
        <div className="bg-[#121211] text-white border border-[#333] rounded-lg p-4 text-[10px] font-mono text-center shadow-lg z-10 w-1/3 flex flex-col gap-2">
          ...
          <span>React<br/>Component<br/>Tree</span>
        </div>
      </div>
    );
  }
    ...
    <div className="w-[min(92vw,1200px)] mx-auto flex flex-col gap-12 lg:gap-16 relative py-12">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-4 border-b border-[var(--border)] pb-8 z-10">
        <div className="max-w-2xl">
          <span className="font-mono text-xs uppercase tracking-[0.2em] font-bold text-[var(--hall-primary)] block mb-4 flex items-center gap-2">
            Hall 04 <span className="w-8 h-px bg-[var(--hall-primary)] opacity-50" /> Proof of Work
          </span>
          ...
          <p className="text-[var(--ink-soft)] font-medium leading-relaxed">
            This space acts as a case-study wall. Each build below represents an explicit problem tackled through code, complete with architectural maps, capability proofs, and live deployments.
          </p>
        </div>
      ...
      <div className="flex flex-col gap-16 md:gap-24">
         {filteredProjects.map((project, index) => (
            <article key={project.id} className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 relative">
               ...
               <div className="lg:col-span-5 flex flex-col gap-6">
                  <div>
                    <div className="flex items-center gap-3 mb-3">
                       ...
                       <span className={`px-2 py-0.5 font-mono text-[10px] rounded font-bold uppercase tracking-wider border ${project.status === 'Active Build' ? 'bg-[var(--orange)]/10 text-[var(--orange)] border-[var(--orange)]/20' : 'bg-[var(--ink-muted)]/10 text-[var(--ink-muted)] border-[var(--ink-muted)]/20'}`}>
                         {project.status}
                       </span>
                    </div>
                    ...
                    <div className="flex gap-2 mb-8">
                      {project.liveUrl && (
                        <a href={project.liveUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-white border border-[var(--border)] text-xs font-bold text-[var(--museum-ink)] hover:text-[var(--hall-primary)] hover:border-[var(--hall-primary)] hover:shadow-sm transition-all focus-visible:ring-2 focus-visible:ring-[var(--hall-primary)]">
                          <ExternalLink className="w-3.5 h-3.5" /> Site deployed
                        </a>
                      )}
                      {project.repoUrl && (
                        <a href={project.repoUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-[#111] text-white border border-[#111] text-xs font-bold hover:bg-[var(--hall-primary)] hover:border-[var(--hall-primary)] transition-all focus-visible:ring-2 focus-visible:ring-[var(--hall-primary)]">
                  ...
                  {/* Tech Stack */}
                  <div>
                    <h4 className="text-[10px] font-mono uppercase font-bold tracking-widest text-[var(--ink-muted)] mb-3">Stack Deployed</h4>
                    <div className="flex flex-wrap gap-2">
                      {project.techStack.map((chip) => (
                        <span key={chip} className="px-2.5 py-1 bg-white border border-[var(--border)] text-[var(--ink-soft)] font-mono text-[10px] font-bold rounded flex items-center tracking-wider hover:border-[var(--hall-primary)] hover:text-[var(--hall-primary)] transition-colors">
                          {chip}
                        </span>
                      ))}
                    </div>
               ...
               <div className="lg:col-span-7 flex flex-col gap-8 bg-white/50 rounded-2xl p-6 lg:p-8 border border-[var(--border)] shadow-sm">
                  ...
                  <div className="grid sm:grid-cols-2 gap-8 mt-4 pt-8 border-t border-[var(--border)]">
                     ...
                     <div className="flex flex-col gap-2">
                        <h4 className="flex items-center gap-2 text-[10px] font-mono uppercase font-bold tracking-widest text-[var(--ink-muted)]">
                           <Activity className="w-3.5 h-3.5 text-[var(--orange)]" /> The Problem
                        </h4>
                     ...
                     <div className="flex flex-col gap-2">
                        <h4 className="flex items-center gap-2 text-[10px] font-mono uppercase font-bold tracking-widest text-[var(--ink-muted)]">
                           <Terminal className="w-3.5 h-3.5 text-[var(--cyan)]" /> System Solutions
                        </h4>
                        <ul className="space-y-2">
                           {project.built.slice(0, 4).map((item, i) => (
                              <li key={i} className="text-xs font-medium text-[var(--ink-soft)] leading-snug flex items-start gap-2">
                                 <span className="w-1.5 h-1.5 rounded-full bg-[var(--cyan)] mt-1 shrink-0" />
                                 {item}
                              </li>
                           ))}
         ...
         {filteredProjects.length === 0 && (
           <div className="py-24 text-center">
             <p className="font-mono text-sm uppercase tracking-widest text-[var(--ink-muted)]">No architectures match the filter.</p>
           </div>
         )}
      </div>
    </div>
